...
let pendingState = null  // Tracks target state to apply after animation
...
const containerStyle = computed(() => {
  ...
  // Expanded: compute height based on card count
  ...
  // Height = rows * card height + (rows-1) * gap + small bottom padding
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Count cards in current row to center each row
    ...
    // Expand downward with extra spacing from the header
    ...
    const x = offset * 35
    // Position close to the header while maintaining comfortable spacing
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Always record the latest target state, even mid-animation
        pendingState = shouldExpand

        // Cancel any previous debounce timer (new scroll intent overrides old)
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }

        // If animating, just record the state and handle it after animation ends
        if (isAnimating) return

        // Target state equals current state — nothing to do
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }

        // Debounce state transitions to prevent rapid flickering.
        // Short delay (50ms) when expanding, longer (200ms) when collapsing for stability.
        ...
        expandDebounceTimer = setTimeout(() => {
          ...
          // Release lock after animation; check for queued state changes
          setTimeout(() => {
            isAnimating = false

            // After animation, check for any new pending state
            if (pendingState !== null && pendingState !== isExpanded.value) {
              // Brief extra delay to avoid switching too quickly
              expandDebounceTimer = setTimeout(() => {
                if (pendingState !== null && pendingState !== isExpanded.value) {
                  isAnimating = true
                  isExpanded.value = pendingState
    ...
    },
    {
      ...
      // Shrink viewport bottom inward so more scrolling is required to trigger expand
...
// Reload history when navigating back to home
...
/* Simplified layout when no projects */
...
.grid-pattern {
  ...
  /* Anchored top-left so height changes only extend the bottom, keeping existing grid fixed */
