...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Max rounds passed in from SimulateView
  minutesPerRound: {
    type: Number,
...
const doStartSimulation = async () => {
  ...
  // Reset all state first to avoid carrying over from the previous simulation
  resetAllState()
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Cleared previous simulation logs, starting fresh')
      }
...
// Track previous round number to detect changes and emit log entries
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Detect simulation completion (via runner_status or platform completed flag)
      const isCompleted = data.runner_status === 'completed' || data.runner_status === 'stopped'

      // Extra check: detect twitter_completed if runner_status hasn't updated yet
...
// Check if the Twitter platform has completed
const checkPlatformsCompleted = (data) => {
  if (!data) return false
  const twitterCompleted = data.twitter_completed === true
  const twitterEnabled = (data.twitter_actions_count > 0) || data.twitter_running || twitterCompleted
  if (!twitterEnabled) return false
  return twitterCompleted
}
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      // Fetch the full action list using all_actions
      ...
      // Do not auto-scroll — allow the user to freely navigate the timeline
    }
  } catch (err) {
...
onMounted(() => {
  addLog('Simulation monitor initialized')
  if (props.simulationId) {
    doStartSimulation()
  }
})
