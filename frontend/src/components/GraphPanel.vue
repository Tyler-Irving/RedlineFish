    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">A small amount of content is still processing — try refreshing the graph shortly.</span>
...
const wasSimulating = ref(false) // tracks whether we were previously simulating
...
// Watch isSimulating for changes to detect when simulation ends
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // Transitioned from simulating to not simulating — show the finished hint
...
const renderGraph = () => {
  ...
  // Count edges between each pair of nodes and collect self-loops
  tempEdges.forEach(e => {
  ...
  // Track how many edges we have processed for each node pair
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // nodes whose self-loop has already been processed
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // If the edge direction is reversed relative to canonical, flip the curvature
      // so all edges are distributed in the same reference frame without overlapping
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Weak gravity toward center so disconnected clusters stay visible
