              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- Show a special indicator when this is the final answer -->
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const subQSection = text.match(/### Sub-Queries\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: 1. **Agent_345（index=1）**: as a representative of...
        ...
        // e.g.: - Selected Agent_601（index 0）: as a parent group representative...
        if (!headerMatch) {
        ...
        // e.g.: - **Agent_601（index 0）**: as a parent group representative...
        if (!headerMatch) {
        ...
        if (name) {
          ...
          // Continuation of a reason (exclude trailing summary paragraphs)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        // Split questions by numbered prefix
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // Handle the case where the first question starts with "1."
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, 'Simulation Scenario: '),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Sub-queries generated via drift search'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Try to split an answer by question number
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // Fall back to "number." format if no matches
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
      ...
      // If no numbering found, or only one segment, return the whole text
      if (matches.length <= 1) {
    ...
    return () => h('div', { class: 'interview-display' }, [
      ...
      props.result.interviews.length > 0 && h('div', { class: 'interview-detail' }, [
        ...
        // Q&A Conversation Thread - alternating Q/A style
...
const renderMarkdown = (content) => {
  ...
  // Strip a leading h2 (## xxx) since the section title is already shown in the outer UI
  ...
  // Remove <p><br> immediately before a block element (caused by extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Remove consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Remove <br> between a block closing tag and the next paragraph/div
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix numbering for non-consecutive ordered lists: when single-item <ol>s are separated by paragraph content, keep the counter incrementing
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic is handled in the nextTick after the loop
          }
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // If complete, scroll to top; otherwise scroll to bottom to follow new logs
            if (isComplete.value) {
...
// Extract the final answer content from an LLM response for a section
const extractFinalContent = (response) => {
  ...
  // Try finding content after "Final Answer:" (multiple formats supported)
  ...
  // If the response starts with ## / # / >, it may be direct markdown content
  const trimmedResponse = response.trim()
  if (trimmedResponse.match(/^[#>]/)) {
    return trimmedResponse
  }

  // If content is long and contains markdown, try stripping the reasoning preamble
  if (response.length > 300 && (response.includes('**') || response.includes('>'))) {
    // Remove a leading Thought: reasoning block
...
.section-number {
  ...
  font-size: 16px;
  color: #9CA3AF; /* dark gray, does not change with section state */
  font-weight: 500;
}
