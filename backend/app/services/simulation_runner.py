...
# Tracks whether the cleanup handler has been registered
...
class SimulationRunState:
    ...
    # Twitter round and simulated time
...
class SimulationRunner:
    ...
    Responsibilities:
    1. Run OASIS simulations in a background subprocess
    ...
    # Graph memory update config — simulation_id -> enabled
    _graph_memory_enabled: Dict[str, bool] = {}
    
    @classmethod
    def get_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        """Return the run state for the given simulation."""
    ...
    def start_simulation(
        cls,
        simulation_id: str,
        platform: str = "parallel",  # twitter / parallel
        max_rounds: int = None,  # max rounds to run (optional, truncates long simulations)
        ...
        Args:
            ...
            platform: Run platform (twitter/parallel)
            max_rounds: Maximum rounds to run (optional, truncates long simulations)
            enable_graph_memory_update: Whether to push agent activity to Zep graph dynamically
        ...
        try:
            ...
            # Open main log file to avoid filling stdout/stderr pipe buffers (which would block the subprocess)
            main_log_path = os.path.join(sim_dir, "simulation.log")
            main_log_file = open(main_log_path, 'w', encoding='utf-8')

            # Set subprocess env vars to enforce UTF-8 on Windows.
            # This fixes third-party libraries (e.g. OASIS) that open files without specifying encoding.
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'          # Python 3.7+: makes all open() default to UTF-8
            env['PYTHONIOENCODING'] = 'utf-8' # ensures stdout/stderr use UTF-8

            # Set cwd to the simulation directory (databases etc. are written there).
    ...
    def _monitor_simulation(cls, simulation_id: str):
        """Monitor the simulation subprocess and parse action logs."""
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def _read_action_log(
        ...
        """
        Read an action log file incrementally.
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # if two were run, both must be done.
                                ...
                                # Update round info from round_end event
                                    ...
                                    state.simulated_hours = state.twitter_simulated_hours
                            ...
                            # If graph memory update is enabled, push the activity to Zep
                            if graph_updater:
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Platform enablement is inferred from whether the corresponding actions.jsonl file exists.
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            # Because start_new_session=True was used, the PGID equals the main PID.
            ...
            try:
                ...
                # Force kill if SIGTERM was not handled in time
    ...
    def _read_actions_from_file(
        ...
        Args:
            ...
            default_platform: Platform to use when the record has no platform field
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Use the record's platform field if present, otherwise fall back to default
    ...
    def get_all_actions(
        ...
        # Read Twitter action file (platform is auto-set to "twitter" by file path)
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        ...
        # Fall back to the old single-file format if per-platform files don't exist
        if not actions:
    ...
    def get_timeline(
        ...
        """
        Return simulation timeline summarised by round.
        ...
        for round_num in sorted(rounds.keys()):
            ...
            result.append({
                ...
                "total_actions": r["twitter_actions"],
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Delete run logs for a simulation (use when forcing a restart).
        ...
        # Platform directories containing action logs to clean
        dirs_to_clean = ["twitter"]
    ...
    def cleanup_all_simulations(cls):
        ...
        Called at server shutdown to ensure all child processes are cleaned up.
        """
        ...
        # Skip silently if there is nothing to clean up
        ...
        # Copy dict to avoid mutating while iterating
    ...
    def register_cleanup(cls):
        ...
        Call once at Flask app startup to ensure all simulation processes are
        ...
        # WERKZEUG_RUN_MAIN=true identifies the reloader child process.
        # In non-debug mode the env var is absent, and we always register.
        is_reloader_process = os.environ.get('WERKZEUG_RUN_MAIN') == 'true'
        is_debug_mode = os.environ.get('FLASK_DEBUG') == '1' or os.environ.get('WERKZEUG_RUN_MAIN') is not None

        # In debug mode: only register in the reloader child; in production: always register
        if is_debug_mode and not is_reloader_process:
            _cleanup_registered = True  # mark as registered so the child doesn't try again
            return
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            # Invoke the original handler so Flask can exit normally
            if signum == signal.SIGINT and callable(original_sigint):
                ...
                if callable(original_sighup):
                    ...
                    # Default behaviour: exit cleanly
                    sys.exit(0)
            else:
                # Original handler is not callable (e.g. SIG_DFL): use default behaviour
        ...
        try:
            signal.signal(signal.SIGTERM, cleanup_handler)  # default kill signal
            signal.signal(signal.SIGINT, cleanup_handler)   # Ctrl+C
            if has_sighup:
                signal.signal(signal.SIGHUP, cleanup_handler)  # terminal close (Unix)
        except ValueError:
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Return detailed status information for the simulation environment.

        Args:
            simulation_id: Simulation ID

        Returns:
            Status dict containing: status, twitter_available, timestamp
        """
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Target platform (optional, fixed to "twitter")
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"Simulation environment is not running or has shut down — cannot interview: {simulation_id}")
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, fixed to "twitter")
            timeout: Timeout in seconds
    ...
    def interview_all_agents(
        ...
        Args:
            simulation_id: Simulation ID
            prompt: Question to ask (all agents receive the same question)
            platform: Target platform (optional, fixed to "twitter")
            timeout: Timeout in seconds
        ...
        # Load all agent configs from the config file
        config_path = os.path.join(sim_dir, "simulation_config.json")
        if not os.path.exists(config_path):
    ...
    def close_simulation_env(
        ...
        Sends a close-environment command so the simulation can exit the command-wait
        loop gracefully.
    ...
    def get_interview_history(
        ...
        """
        Return interview history records (read from the database).

        Args:
            simulation_id: Simulation ID
            platform: Platform type (fixed to "twitter")
            agent_id: Agent ID to filter on (optional)
            limit: Maximum records to return per platform
        ...
        # Cap total results when multiple platforms were queried
