...
Wraps graph search, node reads, edge queries, and other tools for use by the Report Agent.

Core retrieval tools (optimised):
1. InsightForge (deep insight retrieval) — most powerful hybrid retrieval; automatically
...
class InsightForgeResult:
    ...
    Contains retrieval results for multiple sub-questions plus a consolidated analysis.
    """
    query: str
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                if skip:
                    continue
                # Truncate overly long content (on sentence boundary, not hard-cut)
...
class ZepToolsService:
    ...
    Core retrieval tools (optimised):
    1. insight_forge — deep insight retrieval (most powerful; auto-generates sub-questions,
    ...
    def __init__(self, api_key: Optional[str] = None, llm_client: Optional[LLMClient] = None):
        ...
        logger.info("ZepToolsService initialised")
    ...
    def _local_search(
        ...
        def match_score(text: str) -> int:
            """Compute a match score between text and the query."""
            if not text:
                return 0
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        try:
            ...
            result = []
            for edge in all_edges:
                # Include edge if the node appears as source or target
    ...
    def get_entities_by_type(
        ...
        filtered = []
        for node in all_nodes:
            # Check whether the node's labels include the requested type
    ...
    def get_entity_summary(
        ...
        """
        Get a relationship summary for a specified entity.
    ...
    def get_simulation_context(
        ...
        Performs a comprehensive search for information related to the simulation requirement.
        ...
        # Filter to entities that have actual custom type labels (not plain Entity nodes)
        entities = []
        for node in all_nodes:
    ...
    def insight_forge(
        ...
        the question and searches across multiple dimensions:
        ...
        5. Consolidate all results into a deep insight.
        ...
        # Step 3: Extract relevant entity UUIDs from edges; fetch only those entities
        # (avoids fetching all nodes)
        entity_uuids = set()
...
返回JSON格式的子问题列表。"""
    ...
    def panorama_search(
        ...
        2. Retrieve all edges (including expired/invalidated ones).
        3. Classify and organise current-valid vs historical information.

        Suitable for scenarios where you need the full picture of an event or want
        to track how it evolved over time.
    ...
    def interview_agents(
        ...
        1. Auto-reads the persona file to learn about all simulated agents.
        2. Uses the LLM to analyse the interview requirement and intelligently select
           the most relevant agents.
        3. Uses the LLM to generate interview questions.
        4. Calls /api/simulation/interview/batch to conduct real interviews
        ...
        (OASIS environment must not be shut down).

        Use cases:
        - Getting opinions on an event from different role perspectives.
        - Collecting views and viewpoints from multiple parties.
        - Obtaining real responses from simulated agents (not LLM-simulated).

        Args:
            simulation_id: Simulation ID (used to locate the persona file and call the interview API)
            interview_requirement: Interview requirement description (unstructured, e.g.
                "understand students' views on the event")
        ...
        if not profiles:
            ...
            result.summary = "No agent persona file found to interview"
            return result
        ...
        # Step 3: Generate interview questions (if not already provided)
        ...
        try:
            ...
            result.summary = f"Interview failed: {str(e)}. The simulation environment may have been shut down; please ensure OASIS is running."
            return result
        except Exception as e:
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Strip any JSON tool-call wrapper from an agent's response and extract the actual content."""
    ...
    def _load_agent_profiles(self, simulation_id: str) -> List[Dict[str, Any]]:
        ...
        # Build the path to the persona file
