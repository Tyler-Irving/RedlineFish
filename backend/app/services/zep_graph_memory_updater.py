...
Dynamically updates agent activity from simulations into the Zep knowledge graph.
"""
...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        ...
        """
        # Generate a different description depending on the action type
        action_descriptions = {
    ...
    def _describe_quote_post(self) -> str:
        """Quote post — includes original content, author information, and the quote comment."""
        original_content = self.action_args.get("original_content", "")
        original_author = self.action_args.get("original_author_name", "")
    ...
    def _describe_search_user(self) -> str:
        """Search for a user — includes the search keyword."""
...
class ZepGraphMemoryUpdater:
    ...
    Monitors the simulation's actions log file and updates new agent
    activities into the Zep graph in real time.
    Activities are grouped by platform; each platform accumulates BATCH_SIZE
    activities before sending them to Zep in bulk.

    All meaningful actions are written to Zep. The action_args field contains
    ...
    # Batch send size (activities to accumulate per platform before sending)
    BATCH_SIZE = 5
    ...
    def add_activity(self, activity: AgentActivity):
        ...
        action_args will contain full context information (e.g. post text, usernames).
        ...
        while self._running or not self._activity_queue.empty():
            try:
                ...
                try:
                    ...
                    # Add the activity to its platform's buffer
    ...
    def _flush_remaining(self):
        """Flush remaining activities from the queue and platform buffers."""
        # First, drain any remaining activities from the queue into the buffers
        ...
        # Then send any remaining buffered activities per platform (even if below BATCH_SIZE)
        with self._buffer_lock:
