...
simulation requirement, document content, and graph entities.
...
class AgentActivityConfig:
    ...
    # Posting frequency (expected posts per simulated hour)
...
class TimeSimulationConfig:
    ...
    # Time represented by each round (simulated minutes) — 60 min default for faster time progression
    ...
    # Peak hours (19–22, most active for Chinese users)
    peak_hours: List[int] = field(default_factory=lambda: [19, 20, 21, 22])
    peak_activity_multiplier: float = 1.5

    # Off-peak hours (0–5, near-zero activity)
...
class PlatformConfig:
    ...
    # Viral spread threshold (interactions required to trigger amplification)
...
class SimulationConfigGenerator:
    ...
    Uses an LLM to analyse the simulation requirement, document content, and
    graph entities, then automatically produces the optimal simulation parameters.
    ...
    def _build_context(
        ...
        """Build LLM context, truncated to MAX_CONTEXT_LENGTH."""
    ...
    def _summarize_entities(self, entities: List[EntityNode]) -> str:
        """Generate a concise entity summary grouped by type."""
        lines = []
    ...
    def _fix_truncated_json(self, content: str) -> str:
        """Repair JSON that was cut off at a token limit."""
    ...
    def _try_fix_config_json(self, content: str) -> Optional[Dict[str, Any]]:
        ...
        if json_match:
            ...
            try:
                ...
                # Strip all control characters as a last resort
...
- reasoning (string): 简要说明为什么这样配置"""
    ...
    def _get_default_time_config(self, num_entities: int) -> Dict[str, Any]:
        """Return default time config (Chinese daily schedule)."""
        return {
            "total_simulation_hours": 72,
            "minutes_per_round": 60,  # 1 hour per round for faster time progression
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Parse time config result, clamping agents_per_hour to total agent count."""
        agents_per_hour_min = result.get("agents_per_hour_min", max(1, num_entities // 15))
        agents_per_hour_max = result.get("agents_per_hour_max", max(5, num_entities // 5))

        # Clamp to total agent count
        ...
        return TimeSimulationConfig(
            ...
            off_peak_activity_multiplier=0.05,  # Near-zero late-night activity
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        """
        Assign a suitable poster agent to each initial post based on its poster_type.
        """
        ...
        # Type alias map (handles variant formats from the LLM)
        type_aliases = {
        ...
        for post in event_config.initial_posts:
            ...
            # 3. Fall back to the highest-influence agent if no match found
            if matched_agent_id is None:
                logger.warning(f"No agent found for type '{poster_type}', falling back to highest-influence agent")
                if agent_configs:
    ...
    def _generate_agent_configs_batch(
        ...
        """Generate agent configs for a single batch."""

        # Build entity list with truncated summaries
        entity_list = []
...
{{
    "agent_configs": [
        ...
        for i, entity in enumerate(entities):
            ...
            # Fall back to rule-based generation if LLM didn't produce a config
            if not cfg:
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate a single agent config using rule-based defaults (Chinese daily schedule)."""
