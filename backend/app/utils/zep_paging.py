...
This module wraps the auto-pagination logic (with per-page retry) and returns
the complete list transparently to callers.
"""
...
def fetch_all_nodes(
    ...
    """Paginate through all graph nodes, returning at most max_items (default 2000). Each page includes retry logic."""
    all_nodes: list[Any] = []
...
def fetch_all_edges(
    ...
    """Paginate through all graph edges and return the full list. Each page includes retry logic."""
    all_edges: list[Any] = []
    cursor: str | None = None
    page_num = 0
