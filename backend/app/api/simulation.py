...
def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimize an interview prompt by prepending a prefix that prevents the Agent
    from calling tools.
    ...
    Returns:
        Optimized question with prefix
    """
    if not prompt:
        return prompt
...
def get_graph_entities(graph_id: str):
    ...
    Only returns nodes that match pre-defined entity types (nodes whose Labels
    are not just "Entity").
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    2. Required files exist: twitter_profiles.csv, simulation_config.json
    ...
    # Required files list (scripts are in backend/scripts/ and not included here)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # - running: simulation is running, so preparation was completed earlier
...
def prepare_simulation():
    """
    Prepare the simulation environment (async task; all parameters are generated
    intelligently by the LLM).
    ...
    4. Generate simulation config intelligently via LLM (with retry logic)
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "prepare_info": {...}            // detail info when already prepared
            }
        }
    """
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    Scans the reports directory for reports matching the given simulation_id.
...
def get_simulation_history():
    ...
    Used for the home page history display. Returns an enriched simulation list
    including project name, description, and run status.
    ...
    try:
        ...
        # Enrich simulation data — reads only from simulation files
        enriched_simulations = []
        for sim in simulations:
            sim_dict = sim.to_dict()
            
            # Fetch simulation config (reads simulation_requirement from simulation_config.json)
            ...
            # Fetch run state (reads user-configured round count from run_state.json)
            run_state = SimulationRunner.get_run_state(sim.simulation_id)
            if run_state:
                sim_dict["current_round"] = run_state.current_round
                sim_dict["runner_status"] = run_state.runner_status.value
                # Use user-configured total_rounds; fall back to recommended rounds if unset
            ...
            # Fetch file list from the associated project (up to 3)
            ...
            # Fetch the associated report_id (latest report for this simulation)
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Get agent profiles for a simulation in real time (for live progress polling during generation).

    Differences from /profiles:
    - Reads the file directly, bypassing SimulationManager
    - Suitable for polling during profile generation
    - Returns additional metadata (file modification time, generation flag)
    ...
    try:
        ...
        # Check whether generation is in progress (inferred from state.json)
        is_generating = False
        total_expected = None
...
def get_simulation_config_realtime(simulation_id: str):
    """
    Get the simulation config in real time (for live progress polling during generation).

    Differences from /config:
    - Reads the file directly, bypassing SimulationManager
    - Suitable for polling during config generation
    - Returns additional metadata (file modification time, generation flag)
    - Returns partial information even before generation completes
...
def get_simulation_config(simulation_id: str):
    """
    Get the simulation config (full LLM-generated configuration).

    Returns:
        - time_config: timing settings (duration, rounds, peak/off-peak periods)
        - agent_configs: per-agent activity settings (activeness, post frequency, stance, etc.)
        - event_config: event settings (initial posts, hot topics)
        - platform_configs: platform settings
        - generation_reasoning: LLM reasoning notes for the config
    """
    try:
        manager = SimulationManager()
...
def download_simulation_script(script_name: str):
    """
    Download a simulation script file (shared scripts located in backend/scripts/).
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "max_rounds": 100,                     // optional: cap on simulation rounds
            "enable_graph_memory_update": false,   // optional: push agent activity to Zep graph memory
            "force": false                         // optional: force restart (stops running sim and clears logs)
        }

    About force:
        - When enabled, stops and clears run logs if a simulation is running or completed
    ...
    About enable_graph_memory_update:
        - When enabled, all agent activity (posts, comments, likes, etc.) is written to the Zep graph in real time
        - Allows the graph to "remember" the simulation for later analysis or AI chat
        - Requires the linked project to have a valid graph_id
        - Uses batch updates to minimise API calls
    ...
    try:
        ...
        enable_graph_memory_update = data.get('enable_graph_memory_update', False)  # optional: push to Zep graph
        ...
        if platform not in ['twitter', 'parallel']:
            return jsonify({
                "success": False,
                "error": f"Invalid platform: {platform}. Allowed: twitter/parallel"
            }), 400
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                # Process does not exist or has already ended — reset status to ready
        ...
        # Resolve graph_id (used for graph memory updates)
        graph_id = None
        if enable_graph_memory_update:
            # Try to get graph_id from simulation state or project
            graph_id = state.graph_id
            if not graph_id:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "Graph memory update requires a valid graph_id. Build the graph first."
                }), 400
...
def get_run_status(simulation_id: str):
    """
    Get the real-time run status for a simulation (for frontend polling).
...
def get_run_status_detail(simulation_id: str):
    """
    Get detailed run status for a simulation (includes all actions).

    Used for real-time activity display in the frontend.

    Query parameters:
        platform: filter by platform (twitter, optional)
    ...
    try:
        ...
        # Fetch per-platform actions
        twitter_actions = SimulationRunner.get_all_actions(
            simulation_id=simulation_id,
            platform="twitter"
        ) if not platform_filter or platform_filter == "twitter" else []

        # Fetch actions for the current round (recent_actions shows the latest round only)
        ...
        # Build result from base run state
...
def get_agent_stats(simulation_id: str):
    ...
    Used for agent activity ranking and action distribution displays in the frontend.
    """
    try:
