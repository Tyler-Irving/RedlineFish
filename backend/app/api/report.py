...
def get_generate_status():
    ...
    try:
        ...
        # If simulation_id is provided, first check whether a completed report exists
        if simulation_id:
            existing_report = ReportManager.get_report_by_simulation(simulation_id)
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
...
def chat_with_report_agent():
    ...
    The Report Agent can autonomously call retrieval tools during the conversation.
...
def get_report_sections(report_id: str):
    ...
    The frontend can poll this endpoint to receive section content as it is
...
def get_console_log(report_id: str):
    ...
    Retrieves console output (INFO, WARNING, etc.) from the report generation process
    in real time. Unlike the agent-log endpoint which returns structured JSON logs,
    this returns plain-text console-style output.
